...
Once your dataframe is looking pretty, the `load` module will help you update a hosted feature service with your new data. The `FeatureServiceUpdater` class represents a single feature layer and its supporting information, allowing you to call the individual methods for the necessary parts of the update process (add, remove, update, or completely truncate and load).
