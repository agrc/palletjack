...
class ServiceUpdater:
    ...
    def _get_fields_from_dataframe(dataframe: pd.DataFrame) -> list[str]:
        ...
        Args:
            dataframe: Dataframe to get fields from

        Returns:
            List of the columns of the dataframe, excluding Shape_Area and Shape_Length
        """
    ...
    def _upload_data(self, dataframe: pd.DataFrame | GeoAccessor, **append_kwargs) -> int:
        ...
        Args:
            dataframe: A dataframe containing data to be added or upserted to the feature layer or table. The fields must match the live fields in name, type, and length (where applicable). For feature layers, the dataframe must have a SHAPE column containing geometries of the same type as the live data.
    ...
    def _upload_gdb(self, zipped_gdb_path: Path) -> arcgis.gis.Item:
        ...
        Args:
            zipped_gdb_path: Path to the zipped gdb

        Raises:
            RuntimeError: If there is an error uploading the gdb to AGOL

        Returns:
            Reference to the resulting Item object in self.gis
        """
    ...
    def _cleanup(self, gdb_item: arcgis.gis.Item, zipped_gdb_path: Path) -> None:
        ...
        Args:
            gdb_item: Reference to the gdb item in self.gis
            zipped_gdb_path: Path to the gdb on disk
    ...
    def truncate_and_load(self, dataframe: pd.DataFrame | GeoAccessor, save_old=False) -> int:
        """Overwrite a hosted feature layer or table by truncating and loading the new data

        When the existing dataset is truncated, a copy is kept in memory as a dataframe. If save_old is set, this is saved as a layer in self.working_dir/backup.gdb with the layer name {name}_{todays_date}.json (foobar_2022-12-31.json).
        ...
        Args:
            dataframe: Spatially enabled dataframe of new data to be loaded
            save_old: Save existing data to backup.gdb in working_dir. Defaults to False
    ...
    def add(self, dataframe: pd.DataFrame | GeoAccessor) -> int:
        ...
        Args:
            dataframe: Dataframe of data to be added
    ...
    def remove(self, delete_oids: list[int]) -> int:
        ...
        This is a wrapper around the arcgis.FeatureLayer/Table.delete methods that adds some sanity checking. The delete operation is rolled back if any of the features/rows fail to delete using (rollback_on_failure=True). This function will raise a RuntimeError as well after delete() returns if any of them fail.

        The sanity checks will raise errors or warnings as appropriate if any of them fail.

        Args:
            delete_oids: List of OIDs to delete
    ...
    def update(self, dataframe: pd.DataFrame | GeoAccessor, update_geometry: bool = True) -> int:
        ...
        Args:
            dataframe: Dataframe of data to be updated
            update_geometry: Whether to update attributes and geometry (True) or just attributes (False). Defaults to True for feature layers and is always False for tables.
