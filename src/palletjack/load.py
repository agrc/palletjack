...
class ServiceUpdater:
    """A base class for FeatureServiceUpdater and TableServiceUpdater"""

    def __new__(cls, *args, **kwargs):
        if cls is ServiceUpdater:
            raise TypeError(f"{cls.__name__} is an abstract class and cannot be instantiated directly.")
        return object.__new__(cls)

    def __init__(self, gis, itemid, working_dir, gdb_item_prefix) -> None:
    ...
    def _upload_data(self, dataframe: pd.DataFrame | GeoAccessor, **append_kwargs) -> int:
        ...
        standalone item. We then call append on the target feature layer or table with this item as the source for the append,
        ...
        Args:
            dataframe: A dataframe containing data to be added or upserted to
            the feature layer or table. The fields must match the live fields in name, type, and length (where applicable). For feature layers, the dataframe must have a SHAPE column containing geometries of the same type as the live data.
        ...
        if "feature_layer" in self.__dict__:
            append = self.feature_layer.append
        else:
    ...
    def _save_to_gdb_and_zip(self, dataframe: pd.DataFrame | GeoAccessor) -> Path:
    ...
    def _truncate_existing_data(self):
        ...
        if "feature_layer" in self.__dict__:
            truncate = self.feature_layer.manager.truncate
        else:
            truncate = self.table.manager.truncate
    ...
    def truncate_and_load(self, dataframe, save_old=False):
        ...
        """
        if "feature_layer" in self.__dict__:
            properties = self.feature_layer.properties
            table_or_layer = self.feature_layer
            index = self.layer_index
        else:
            properties = self.table.properties
            table_or_layer = self.table
            index = self.table_id

        self._class_logger.info("Truncating and loading index `%s` in itemid `%s`", index, self.itemid)
        start = datetime.now()

        #: Save the data to disk if desired
        if save_old:
            self._class_logger.info("Saving existing data to %s", self.working_dir)
            saved_layer_path = utils.save_to_gdb(table_or_layer, self.working_dir)

        fields = ServiceUpdater._get_fields_from_dataframe(dataframe)

        #: Field checks to prevent various AGOL errors
        utils.FieldChecker.check_fields(properties, dataframe, fields, add_oid=False)
...
class FeatureServiceUpdater(ServiceUpdater):
    ...
    update_features, and truncate_and_load.
    ...
    def __init__(self, gis, itemid, working_dir=None, layer_index=0, gdb_item_prefix="palletjack"):
        super().__init__(gis, itemid, working_dir=working_dir, gdb_item_prefix=gdb_item_prefix)
        self.feature_layer = arcgis.features.FeatureLayer.fromitem(gis.content.get(itemid))
        self.layer_index = layer_index

    def add_features(self, dataframe: pd.DataFrame.spatial):
        ...
        self._class_logger.info("Adding items to layer `%s` in itemid `%s` in-place", self.layer_index, self.itemid)
        fields = ServiceUpdater._get_fields_from_dataframe(dataframe)
    ...
    def remove_features(self, delete_oids):
        ...
        self._class_logger.info("Deleting features from layer `%s` in itemid `%s`", self.layer_index, self.itemid)
    ...
    def update_features(self, dataframe, update_geometry=True):
        ...
        self._class_logger.info("Updating layer `%s` in itemid `%s` in-place", self.layer_index, self.itemid)
...
class TableServiceUpdater(ServiceUpdater):
    """Update an AGOL Standalone Table service with data from a pandas DataFrame.

    This class represents the table that will be updated and stores a reference to the table and it's
    containing gis. It contains four methods for updating the layer's data: add_rows, remove_rows,
    update_rows, and truncate_and_load.

    It is the client's responsibility to separate out the new data into these different steps. If the extract/transform
    stages result in separate groups of records that need to be added, deleted, and updated, the client must call the
    three different methods with dataframes containing only the respective records for each operation.

    The method used to upload the data to AGOL saves the updated data as a new table named upload in working_dir/upload.
    gdb, zips the gdb, uploads it to AGOL (with the item name `gdb_item_prefix Temporary gdb upload`), and then uses
    this as the source data for a call to the tables's .append() method. The geodatabase upload.gdb will be
    created in working_dir if it doesn't already exist. Ideally, working_dir should be a TemporaryDirectory unless
    persistent access to the gdb is desired.
    """

    def __init__(self, gis, itemid, working_dir=None, table_id=0, gdb_item_prefix="palletjack"):
        super().__init__(gis, itemid, working_dir=working_dir, gdb_item_prefix=gdb_item_prefix)
        self.table = arcgis.features.Table.fromitem(gis.content.get(itemid))
        self.table_id = table_id

    def add_rows(self, dataframe: pd.DataFrame):
        """Adds new rows to existing hosted table from new dataframe.

        The new dataframe's columns and data must match the existing data's fields in name, type, and allowable length. Live fields that are not nullable and
        don't have a default value must have a value in the new data; missing data in these fields will raise an error.
        ...
        Raises:
            ValueError: If the new field and existing fields don't match, the new data contains null fields, the new data exceeds the existing field lengths, or a specified field is missing from either new or live data.
        ...
        #: Field checks to prevent various AGOL errors
        utils.FieldChecker.check_fields(self.table.properties, dataframe, fields, add_oid=False)
    ...
    def remove_rows(self, delete_oids):
        """Deletes features from a hosted feature layer based on comma-separated string of Object IDs
        ...
        self._class_logger.info("Deleting features from layer `%s` in itemid `%s`", self.table_id, self.itemid)
        self._class_logger.debug("Delete string: %s", delete_oids)
        ...
        utils.DeleteUtils.check_for_empty_oid_list(oid_numeric, delete_oids)
        delete_string = ",".join([str(oid) for oid in oid_numeric])
        num_missing_oids = utils.DeleteUtils.check_delete_oids_are_in_live_data(delete_string, oid_numeric, self.table)

        #: Note: apparently not all services support rollback:
        #: https://developers.arcgis.com/rest/services-reference/enterprise/delete-features.htm
        deletes = utils.retry(
            self.table.delete_features,
            deletes=delete_string,
            rollback_on_failure=True,
        )
    ...
    def update_rows(self, dataframe, update_geometry=True):
        """Updates existing features within a hosted feature layer using OBJECTID as the join field.
        ...
        self._class_logger.info("Updating layer `%s` in itemid `%s` in-place", self.table_id, self.itemid)
        ...
        if not update_geometry:
            self._class_logger.debug("Attribute-only update; inserting null geometries")
            dataframe["SHAPE"] = utils.get_null_geometries(self.table.properties)

        #: Field checks to prevent various AGOL errors
        utils.FieldChecker.check_fields(self.table.properties, dataframe, fields, add_oid=True)
